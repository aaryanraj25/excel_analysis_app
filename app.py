...
def load_and_clean_data(files):
    ...
    for file in files:
        ...
        # Keep both Account and A/C Holder Name columns
        df['Account'] = df['Account']  # Ensure Account column exists
...
def create_heatmap(df):
    ...
    fig = px.imshow(numeric_data.T, 
                    ...
                    labels={'y': 'Month', 'x': 'Account'},  # Changed to Account
                    aspect='auto')
    return fig
...
def main():
    ...
    if uploaded_files:
        ...
        with col2:
            st.plotly_chart(create_state_distribution_pie(df))
        
        # Monthly A/C Holder Distribution with legend on right
        st.header('Monthly A/C Holder Distribution')
        st.plotly_chart(create_monthly_holder_comparison(df), use_container_width=True)
