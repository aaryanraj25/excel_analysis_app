...
def create_account_holder_bar_chart(df):
    """Create an interactive bar chart for selected account holders"""
    # Debug information
    ...
    try:
        # Get list of unique account holders
        account_holders = df[holder_column].unique().tolist()
        
        # Create multiselect for account holders
        ...
        # Filter data for selected account holders
        ...
        fig = px.bar(melted_data,
                     ...
                     y='Packets',
                     color=holder_column,
                     title='Monthly Distribution by Account Holder (Bar Chart)',
                     barmode='group')
